...
void MyGLWidget::initializeGL ()
{
  ...
  glUniform4fv(focusLoc,1,&focus[0]);
}
...
void MyGLWidget::carregaShaders()
{
  ...
  focusLoc = glGetUniformLocation (program->programId(), "focusSCO");
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_F: { // canvia òptica entre perspectiva i axonomètrica
        glm::vec4 focus;
        llumSCA = not llumSCA;
        if (llumSCA) focus = View * glm::vec4(posiFocus, 1.0); //LLUM D'ESCENA
        else focus = glm::vec4 (posiFocus, 1.0); //LLUM D'OBSERVADOR
        glUniform4fv(focusLoc,1,&focus[0]);
      break;
    }
