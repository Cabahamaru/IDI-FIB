...
void MyGLWidget::initializeGL ()
{
  ...
  posiFocus = glm::vec3(llumx,1,1);
  /*glUniform3fv(colFocusLoc,1,&colorFocus[0]);
  glUniform3fv(posFocusLoc,1,&posiFocus[0]);
  */
  iniEscena();
  iniCamera();
  glm::vec4 focus = View * glm::vec4(posiFocus, 1.0); //LLUM D'ESCENA
  glUniform4fv(posFocusLoc,1,&focus[0]);
}
...
void MyGLWidget::carregaShaders()
{
  ...
  posFocusLoc = glGetUniformLocation(program->programId(), "focusSCO");
...
void MyGLWidget::viewTransform ()
{
  ...
  if (llumSCA) {
      glm::vec4 focus = View * glm::vec4(posiFocus, 1.0);
      glUniform4fv(posFocusLoc,1,&focus[0]);
  }
}
  ...
  switch (event->key()) {
    ...
    case Qt::Key_K: { // canvia òptica entre perspectiva i axonomètrica
        if (llumSCA) {
            llumx -= 0.1;
            posiFocus = glm::vec3(llumx,1,1);
            focus = View * glm::vec4(posiFocus, 1.0);
        }
        else {
            llumx2 -= 0.1;
            posiFocus = glm::vec3(llumx2,0,0);
            focus = glm::vec4 (posiFocus, 1.0);
        }
        glUniform4fv(posFocusLoc,1,&posiFocus[0]);
      break;
    }
    ...
    case Qt::Key_F: { // canvia òptica entre perspectiva i axonomètrica
        ...
        if (llumSCA) {
            posiFocus = glm::vec3(llumx,1,1);
            focus = View * glm::vec4(posiFocus, 1.0); //LLUM D'ESCENA
        }
        else {
            posiFocus = glm::vec3(llumx,0,0);
            focus = glm::vec4 (posiFocus, 1.0); //LLUM D'OBSERVADOR
        }
        glUniform4fv(posFocusLoc,1,&focus[0]);
      break;
    }
