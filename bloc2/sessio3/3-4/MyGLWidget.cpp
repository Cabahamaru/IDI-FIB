...
{
    float d = radi;
    glm::mat4 view = glm::translate(glm::mat4(1.0f),glm::vec3(0.,0.,-d*2.0f));
...
void MyGLWidget::projectTransform_ortho() 
{
    //glm::perspective(FOV en radians, ra window, znear, zfar)
    //glm::mat4 Proj=glm::perspective((float)M_PI/2.0f, 1.0f, 0.4f, 3.0f);
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e) 
{
   makeCurrent();
    ...
    if (rx < xnew) sx += 0.1;
    else if (rx > xnew) sx -= 0.1;
    if (ry < ynew) sy += 0.1;
    else if (ry > ynew) sy -= 0.1;
