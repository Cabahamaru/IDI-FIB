...
void MyGLWidget::carregaShaders()
{
  ...
  bool3Loc = glGetUniformLocation (program->programId(), "escena");
  bool2Loc = glGetUniformLocation (program->programId(), "x_premut");
  boolLoc = glGetUniformLocation (program->programId(), "es_vaca");
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_X: { 
      x_premut = !x_premut;
      glUniform1i (bool2Loc,x_premut);
      break;
    }
    case Qt::Key_L: { 
      escena = !escena;
      glUniform1i (bool3Loc,escena);
      if (escena == 1) emit signal_escena();
      else emit signal_camera();
      break;
    }
...
void MyGLWidget::tracta_escena () {
    makeCurrent();
    escena = 1;
    glUniform1i (bool3Loc,escena);
    update();
}
void MyGLWidget::tracta_camera () {
    makeCurrent();
    escena = 0;
    glUniform1i (bool3Loc,escena);
    update();
